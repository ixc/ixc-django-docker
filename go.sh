...
if [[ ! -s requirements.txt.md5 ]] || ! md5sum --status -c requirements.txt.md5 > /dev/null 2>&1; then
	"$PROJECT_VENV_DIR/bin/python" -m pip install --no-cache-dir --no-deps -r <(grep -Ev setuptools requirements.txt)  # Unpin setuptools dependencies. See: https://github.com/pypa/pip/issues/4264
